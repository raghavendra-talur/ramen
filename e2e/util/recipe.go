...
func CreateRecipe(ctx types.TestContext, recipeSpec *config.Recipe) error {
	...
	recipeC1 := getBaseRecipe(ctx)
	ns := ctx.AppNamespace()

	if recipeSpec.CheckHook || recipeSpec.ExecHook {
		recipeC1.Spec.Hooks = getHooks(ns)
	}

	recipeC1.Spec.Groups = getGroups(ns)
	recipeC1.Spec.Workflows = getWorkflows(recipeSpec)
	...
	log.Infof("Recipe %s created in namespace %s on both c1 and c2 clusters", recipeC1.Name, recipeC1.Namespace)
...
func getBaseRecipe(ctx types.TestContext) *recipe.Recipe {
...
func getGroups(namespace string) []*recipe.Group {
	return []*recipe.Group{
		{
			Name:      "rg1",
			Type:      "resource",
...
func getHooks(namespace string) []*recipe.Hook {
	return []*recipe.Hook{
		{
			Name:           "check-hook",
			Type:           "check",
...
func getWorkflow(name string) *recipe.Workflow {
...
func getWorkflows(recipeSpec *config.Recipe) []*recipe.Workflow {
	backup := getWorkflow("backup")
	restore := getWorkflow("restore")
